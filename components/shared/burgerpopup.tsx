    ...
    return (
        <div className="flex flex-col absolute top-0 right-0 bg-[#333d41] w-[150px] h-[300px] [transition:1s] z-[100]">
            <div className="flex mt-[135px] flex-col gap-[2px] text-white font-bold backdrop-filter backdrop-blur-[20px]">
                <div onClick={() => setReviews(true)} className="flex bg-white/20 w-full h-[40px] items-center justify-center">{t('reviews')}</div>
                ...
                <div onClick={() => setReviews(false)} className="flex bg-white/20 w-full h-[40px] items-center justify-center">{t('aboutus')}</div>
            </div>
        </div>
    )
}